...
local function format_message(message, ...)
	...
	local args = { ... }
	local s, n = string.gsub(message, "{}", function(s)
		local arg = table.remove(args, 1) or "nil"
		return cjson.encode(arg)
	end)
	return s
end
...
for i=1,MAX_LEVELS do
	...
	LEVEL_FUNCS[i] = function(self, ...)
		...
		return LOG_MSG(self, level, format_message, ...) -- edit by zhumh
	end
end
