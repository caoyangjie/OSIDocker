...
function Redis:init(datasource)
    ...
    logger:debug("[Redis:init] datasource : " .. cjson.encode(self.datasource))
end

function Redis:getConnect()
    local connect, err = resty_redis:new()
    if not connect then
        logger:error("[Redis:getConnect] failed to create redis : " .. err)
        return nil
    end
    connect:set_timeout(self.datasource.timeout)
    local ok, err = connect:connect(self.datasource.host, self.datasource.port)
    if not ok then
        logger:error("[Redis:getConnect] failed to connect redis : " .. err)
        return nil
    end
...
function Redis:cmd(method, ...)
    ...
    logger:debug("[Redis:cmd] connected to redis ok.")
    -- exec cmd
