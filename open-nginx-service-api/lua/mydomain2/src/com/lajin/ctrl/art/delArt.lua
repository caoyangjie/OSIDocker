...
function delArt(request, response)
    -- 参数校验
    local param = {}
    ...
    ngx.log(logger.i("删除作品输入参数：", cjson.encode(param)))
	...
	if str_util.isNil(art_table)  or  param["uid"] ~= art_table.uid then
		...
		response:writeln(json_util.illegal_argument('作品删除失败'))
        return
	end
	redis_util:close(redis)
	-- 调用内网接口保存用户
    local deleteArtUrl = luastar_config.getConfig("apihost")["inner_url"] .. "/inner/app/art/del"
    local ok, code, resheaders, status, resbody = httpclient.request(deleteArtUrl, "POST", {aid = param["aid"]}, 1500000, {})
    if not ok or _.isEmpty(resbody) then
        response:writeln(json_util.fail("删除作品失败"))
        return
    end
