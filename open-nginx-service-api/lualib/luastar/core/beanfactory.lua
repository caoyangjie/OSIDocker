...
function BeanFactory:init(config_file)
    logger:debug("[BeanFactory:init] file : " .. config_file)
...
function BeanFactory:createBean(id, ctime)
    ...
    if ctime[id] == bean_status.init_ing then
        ...
        ctime[id] = nil
        logger:error("[BeanFactory:createBean] id " .. id .. " has circular dependency.")
        return bean
    end
    ctime[id] = bean_status.init_ing
    local bean_config = self.config[id] or {}
    logger:debug("[BeanFactory:createBean] " .. id .. " config : " .. cjson.encode(bean_config))
    if not bean_config.class then
        bean.status = bean_status.init_fail
        ctime[id] = nil
        logger:error("[BeanFactory:createBean] " .. id .. " config class is null.")
        return bean
    end
    ...
    if not ok then
        bean.status = bean_status.init_fail
        ctime[id] = nil
        logger:error("[BeanFactory:createBean] " .. id .. "  require class fail.")
        ...
        _.each(bean_config.property, function(index, property)
            ...
            if _.isCallable(method) then
                ...
                elseif property.ref then
                    ...
                    logger:warn("[BeanFactory:createBean] " .. id .. " property[" .. property.name .. "] value is nil.")
                end
            else
                logger:warn("[BeanFactory:createBean] " .. id .. " method[" .. property.name .. "] not exist.")
            end
        end)
    end
...
function BeanFactory:getValue(key)
    ...
    if not var then
        logger:debug("[BeanFactory:getValue] key[" .. key .. "] value : " .. key)
        return key
    end
    ...
    if varLen == 1 then
        logger:debug("[BeanFactory:getValue] key[" .. key .. "] value : " .. cjson.encode(val))
        return val
    end
    ...
    logger:debug("[BeanFactory:getValue] key[" .. key .. "] value : " .. cjson.encode(rs))
    return rs
end
