...
function Route:init(config_file)
    logger:debug("[Route:init] file : " .. config_file)
    ...
    logger:info("[Route:init] config_route : " .. cjson.encode(self.config_route))
    -- 初始化拦截器配置
    self.config_interceptor = config["interceptor"] or {}
    logger:info("[Route:init] config_interceptor : " .. cjson.encode(self.config_interceptor))
end

function Route:getRoute(uri)
    if _.isEmpty(uri) then
        logger:warn("[Route:getRoute] uri is nil.")
        return nil
    end
